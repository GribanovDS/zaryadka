    ...
    while (true) {
        ...
        if (r.type === 'SERVER_ACTION' && r.act?.action_id === 'help') {
            ...
            if (exer.includes('hands1')) 
                rsp.msg = `${(appeal === 'official' ? 'Делайте' : 'Делай')} вращения плечами. Для этого ${(appeal === 'official' ? 'положите' : 'положи')} свои руки на плечи и ${(appeal === 'official' ? 'вращайте' : 'вращай')} их в одну сторону. Затем ${(appeal === 'official' ? 'повторяйте' : 'повторяй')} то же самое, только в другую сторону`
                ...
                rsp.msg = `${(appeal === 'official' ? 'Делайте' : 'Делай')} рывки руками вверх и вниз. Для этого одну руку ${(appeal === 'official' ? 'опустите' : 'опусти')} вниз, а другую ${(appeal === 'official' ? 'поднимите' : 'подними')} вверх. После сделанных рывков ${(appeal === 'official' ? 'поменяйте' : 'поменяй')} руки местами`
