...
function* script(r: SberRequest, rs: SberResponse) {
    ...
    phrase = ` Добро пожаловать в приложение «Утренняя Зарядка»! Здесь ${(appeal === 'official' ? 'вы можете' : 'ты можешь')} провести личную зарядку. Для этого сначала нужно выбрать её длительность, а затем повторять за упражнениями на экране. Удачи!`;
    rsp.msg = phrase;
    rsp.data = {type: 'init'};
    const Exercise_Name = () => {
        rsp.msg = 'Я Дима'
    ...
    while (true) {
        ...
        if (r.type === 'SERVER_ACTION' && r.act?.action_id === 'help') {
            ...
            if (command.type === 'greet') {
                rsp.msg =  ` Добро пожаловать в приложение «Утренняя Зарядка»! Здесь ${(appeal === 'official' ? 'вы можете' : 'ты можешь')} провести личную зарядку. Для этого сначала нужно выбрать её длительность, а затем повторять за упражнениями на экране. Удачи!`;
                rsp.data = command;
